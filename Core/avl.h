...
extern void* CbHeapAllocate(unsigned int nBytes, int bZeroInit);
extern void CbHeapFree(void* pBlock);

/* modify this macro to change the prefix */
#define AVL_NAME(name) avl_ ## name
/* memory allocation macros, change as necessary */
#define AVL_ALLOC(variable, type) variable = (type *)CbHeapAllocate(sizeof(type), 0)
#define AVL_FREE(variable) CbHeapFree(variable)
